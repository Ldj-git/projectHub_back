...
router.get('/:message', function (req, res) {
    if(req.params.message === "fail"){
        res.render('login', {
            message:"ID does not registered or password does not match. Please check again."
        });
    }
...
router.post('/process', function(req, res){
    ...
    con.query("SELECT id, pwd FROM user WHERE id=?", [post.id],
